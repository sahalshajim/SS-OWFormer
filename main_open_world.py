...
from datasets.coco import make_coco_transforms, make_coco_transforms_ssl, make_normalize, make_augment
...
def get_args_parser():
    ...
    parser.add_argument('--Fresume', default='', help='resume from Frozen checkpoint')
    ...
    parser.add_argument('--unmatched_boxes',
                        default=False, action='store_true')
    ...
    parser.add_argument('--invalid_cls_logits', default=False,
                        action='store_true', help='owod setting')
    parser.add_argument('--nc_epoch', default=0, type=int)
    parser.add_argument('--num_classes', default=81, type=int)
    parser.add_argument('--backbone', default='resnet50', type=str,
                        help="Name of the convolutional backbone to use")
    parser.add_argument('--data_sub', default='data/OWOD/VOC2007/ImageSets/t1_train.txt')
...
def main(args):
    ...
    if enableSSL :
        ...
        if args.Fresume :
            checkpoint = torch.load(args.Fresume, map_location='cpu')
            missing_keys, unexpected_keys = F_model.module.load_state_dict(
            checkpoint['model'], strict=False)
            unexpected_keys = [k for k in unexpected_keys if not (
                k.endswith('total_params') or k.endswith('total_ops'))]
            if len(missing_keys) > 0:
            ...
            if len(unexpected_keys) > 0:
                print('Unexpected Keys: {}'.format(unexpected_keys))
            print("==== Initialize Frozen Model ====")
        F_criterion = deepcopy(criterion)
        for pg in F_model.parameters():
            pg.requires_grad = False
        for pg in F_criterion.parameters():
            pg.requires_grad = False
        F_model.eval()
        F_criterion.eval()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if enableSSL :
            if args.Fresume :
                print('With Frozen Model')
                train_stats = train_one_epoch(
                    model, criterion, data_loader_train, optimizer, device, epoch, args.nc_epoch, args.clip_max_norm, True, F_model, F_criterion)
            else :
                print('No Frozen Model')
                train_stats = train_one_epoch(
                    model, criterion, data_loader_train, optimizer, device, epoch, args.nc_epoch, args.clip_max_norm, False, F_model, F_criterion)
        else :
            train_stats = train_one_epoch(
...
def get_datasets(args):
    ...
    if args.dataset == 'owod':
        ...
        if enableSSL:
            ...
            dataset_train = OWDetection(args, args.owod_path, ["2007"], image_sets=[args.train_set], transforms=make_coco_transforms_ssl(
                args.train_set), tranfomrs_normalize=make_normalize(args.train_set), tranfomrs_aug=make_augment(args.train_set))
            dataset_val = OWDetection(args, args.owod_path, ["2007"], image_sets=[
                                      args.test_set], transforms=make_coco_transforms(args.test_set))
        else:
            train_set = args.train_set
            test_set = args.test_set
            dataset_train = OWDetection(args, args.owod_path, ["2007"], image_sets=[
                                        args.train_set], transforms=make_coco_transforms(args.train_set))
            dataset_val = OWDetection(args, args.owod_path, ["2007"], image_sets=[
                                      args.test_set], transforms=make_coco_transforms(args.test_set))
    else:
