...
UNK_CLASS = ["unknown"]  # NOTE


VOC_COCO_CLASS_NAMES = tuple(itertools.chain(T1_CLASS_NAMES, T2_CLASS_NAMES, T3_CLASS_NAMES, T4_CLASS_NAMES, UNK_CLASS))  # create splits
...
class OWDetection(VisionDataset):
    ...
    def __init__(self,
                 ...
                 transforms=None,
                 tranfomrs_aug=None,
                 tranfomrs_normalize=None,
                 no_cats=False,
                 filter_pct=-1):
        super(OWDetection, self).__init__(
        ...
        self.transforms_aug = tranfomrs_aug
        self.transforms_normalize = tranfomrs_normalize
        ...
        for year, image_set in zip(years, image_sets):
            ...
            if year == "2007" and image_set == "test":
                year = "2007-test"
            valid_sets = ["t1_train", "t2_train", "t2_train_50", "t3_train", "t4_train",
                          "t2_ft", "t2_ft_50", "t3_ft", "t4_ft", "test", "all_task_test"]
            if year == "2007-test":
            ...
            self.images.extend([os.path.join(image_dir, x + ".jpg")
                               for x in file_names])
            self.annotations.extend(
                [os.path.join(annotation_dir, x + ".xml") for x in file_names])

            self.imgids.extend(self.convert_image_id(
                x, to_integer=True) for x in file_names)
            self.imgid2annotations.update(
                dict(zip(self.imgids, self.annotations)))

        if filter_pct > 0:
            num_keep = float(len(self.imgids)) * filter_pct
            keep = np.random.choice(np.arange(len(self.imgids)), size=round(
                num_keep), replace=False).tolist()

            def flt(l): return [l[i] for i in keep]
        ...
        enableSSL = True
        if enableSSL:
            data_path = self.args.data_sub
            with open(data_path) as file:
                self.data_subset = [self.convert_image_id(
                    line.rstrip(), to_integer=True) for line in file]
    ...
    def __getitem__(self, index):
        ...
        target = dict(
            ...
            labels=torch.tensor([i['category_id']
                                for i in instances], dtype=torch.int64),
            area=torch.tensor([i['area']
                              for i in instances], dtype=torch.float32),
            boxes=torch.as_tensor([i['bbox']
                                  for i in instances], dtype=torch.float32),
        ...
        if self.imgids[index] in self.data_subset:
            target['annotation'] = torch.as_tensor(True)
        else:
            target['annotation'] = torch.as_tensor(False)

        og_target = target.copy()

        if enableSSL and ('test' not in image_set):
            img_aug = self.transforms_aug[-1](img.copy())
            img, target = self.transforms_normalize[-1](img, target)
            img_aug, target_aug = self.transforms_normalize[-1](
                img_aug, og_target)
            imgL = [img, img_aug]
            return imgL, target
        else:
