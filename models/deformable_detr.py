...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if with_box_refine:
            ...
            if self.novelty_cls:
                self.nc_class_embed = nn.ModuleList(
                    [self.nc_class_embed for _ in range(num_pred)])
    ...
    @torch.no_grad()
    def frozen_forward(self, samples: NestedTensor, enableSSL=0):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, enc_feats = self.transformer(
    ...
    def forward(self, samples: NestedTensor, enableSSL=0, samples_aug = None):
        ...
        if enableSSL>0 and samples_aug is not None :
            
            if not isinstance(samples_aug, NestedTensor):
                samples_aug = nested_tensor_from_tensor_list(samples_aug)
            features_a, pos_a = self.backbone(samples_aug)

            srcs_a = []
            masks_a = []
            for l, feat in enumerate(features_a):
                src, mask = feat.decompose()
                srcs_a.append(self.input_proj[l](src))
                masks_a.append(mask)
                assert mask is not None
            if self.num_feature_levels > len(srcs_a):
                _len_srcs = len(srcs_a)
                for l in range(_len_srcs, self.num_feature_levels):
                    if l == _len_srcs:
                        src = self.input_proj[l](features_a[-1].tensors)
                    else:
                        src = self.input_proj[l](srcs_a[-1])
                    m = samples_aug.mask
            ...
            query_embeds_a = None
            if not self.two_stage:
                query_embeds_a = self.query_embed.weight
            hs_aug , _1, _2, _3, _4, _5 = self.transformer(srcs_a, masks_a, pos_a, query_embeds_a)
        ...
        if enableSSL > 0 :
            ...
            A_p1 = self.predictor(A_z1)
            B_p1 = self.predictor(B_z1)

            decoder_raw_aug = hs_aug[-1]
            A_z2 = decoder_raw_aug[0]
            B_z2 = decoder_raw_aug[1]

            A_p2 = self.predictor(A_z2)
            B_p2 = self.predictor(B_z2)
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],
               'resnet_1024_feat': [resnet_1024_feature, enc_feats]}

        if self.novelty_cls:
            out = {'pred_logits': outputs_class[-1], 'pred_nc_logits': output_class_nc[-1],
                   'pred_boxes': outputs_coord[-1], 'resnet_1024_feat': [resnet_1024_feature, enc_feats]}

        if enableSSL == 1:
            out = {'pred_logits': outputs_class[-1], 'pred_nc_logits': output_class_nc[-1],
                   'pred_boxes': outputs_coord[-1], 'resnet_1024_feat': [resnet_1024_feature, enc_feats], 'ssl_out': [A_z1, A_z2, B_z1, B_z2]}
            
        if enableSSL == 2:
            out = {'pred_logits': outputs_class[-1], 'pred_nc_logits': output_class_nc[-1],
                   'pred_boxes': outputs_coord[-1], 'resnet_1024_feat': [resnet_1024_feature, enc_feats], 'ssl_out': [A_z1, A_z2, B_z1, B_z2],'ssl_out_p': [A_p1, A_p2, B_p1, B_p2]}

        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(
                outputs_class, outputs_coord, output_class_nc=None)
            if self.novelty_cls:
                out['aux_outputs'] = self._set_aux_loss(
                    outputs_class, outputs_coord, output_class_nc=output_class_nc, hs=hs, hs_aug=hs_aug)
        if self.two_stage:
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, output_class_nc=None, hs=None, hs_aug=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        # import pdb;pdb.set_trace()
        enableSSL = True
        if output_class_nc is not None and enableSSL and hs_aug is not None:
            xx = [{'pred_logits': a, 'pred_nc_logits': c, 'pred_boxes': b, 'hs':d, 'hs_aug': e}
                  for a, c, b, d, e in zip(outputs_class[:-1], output_class_nc[:-1], outputs_coord[:-1],hs[:-1],hs_aug[:-1])]
        elif output_class_nc is not None :
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, args, num_classes, matcher, weight_dict, losses, invalid_cls_logits, focal_alpha=0.25):
        ...
        self.prev_introduced = args.PREV_INTRODUCED_CLS
    ...
    def loss_NC_labels(self, outputs, targets, indices, num_boxes, current_epoch, owod_targets, owod_indices, log=True):
        ...
        target_classes_o = torch.cat([torch.full_like(
            t["labels"][J], 0) for t, (_, J) in zip(owod_targets, owod_indices)])
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, current_epoch, owod_targets, owod_indices, log=True):
        ...
        if self.unmatched_boxes:
            ...
            target_classes_o = torch.cat(
                [t["labels"][J] for t, (_, J) in zip(owod_targets, owod_indices)])
        else:
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes, current_epoch, owod_targets, owod_indices):
        ...
        if self.unmatched_boxes:
            tgt_lengths = torch.as_tensor(
                [len(v["labels"]) for v in owod_targets], device=device)
        else:
            tgt_lengths = torch.as_tensor(
    ...
    def loss_ssl(self, outputs, targets, indices, num_boxes, current_epoch, owod_targets, owod_indices):

        if 'ssl_out' in outputs :

            A_z1, A_z2, B_z1, B_z2 = outputs['ssl_out']

            lamb = 0.005
            scale_loss = 0.1

            # SSL Loss for imgA
            ssl_A = barlow_loss_func(A_z1, A_z2, lamb=lamb, scale_loss=scale_loss)
            # SSL Loss for imgB
            ssl_B = barlow_loss_func(B_z1, B_z2, lamb=lamb, scale_loss=scale_loss)

            loss_ssl = ssl_A + ssl_B

            if 'ssl_out_f' in outputs :
                A_p1, A_p2, B_p1, B_p2 = outputs['ssl_out_p']
                A_fz1,A_fz2,B_fz1,B_fz2 = outputs['ssl_out_f']
                # Distill Loss imgA 
                distil_A = (
                    barlow_loss_func (
                        A_p1, A_fz1, lamb, scale_loss 
                    ) 
                    +
                    barlow_loss_func (
                        A_p2, A_fz2, lamb, scale_loss 
                    ) 
                ) / 2
                ...
                distil_B = (
                    barlow_loss_func (
                        B_p1, B_fz1, lamb, scale_loss 
                    ) 
                    +
                    barlow_loss_func (
                        B_p2, B_fz2, lamb, scale_loss 
                    ) 
                ) / 2

                loss_distill = (distil_A + distil_B)/4
                loss_ssl = loss_ssl + loss_distill
          
            losses = {"loss_ssl": loss_ssl}
            return losses
        else :
            decoder_raw = outputs['hs']
            A_z1 = decoder_raw[0]
            B_z1 = decoder_raw[1]

            decoder_raw_aug = outputs['hs_aug']
            A_z2 = decoder_raw_aug[0]
            B_z2 = decoder_raw_aug[1]
    ...
    def _get_src_single_permutation_idx(self, indices, index):
        ...
        batch_idx = [torch.full_like(src, i)
                     for i, src in enumerate(indices)][0]
        src_idx = indices[0]
        return batch_idx, src_idx
    ...
    def forward(self, samples, outputs, targets, epoch, enableSSL = 0):
        ...
        if enableSSL == 3 :
            ...
            for logits in outputs :
                if logits in ['pred_logits','pred_nc_logits','pred_boxes']:
        ...
        if self.unmatched_boxes and epoch >= loss_epoch:
            ...
            enc_feats_512 = outputs['resnet_1024_feat'][1][0]
            enc_feats_1024 = outputs['resnet_1024_feat'][1][1]
            enc_feats_2048 = outputs['resnet_1024_feat'][1][2]

            if enableSSL == 3 :
                enc_feats_1024 = enc_feats_1024[1:]
            ...
            res_feat = enc_feats_1024
            queries = torch.arange(outputs['pred_logits'].shape[1])
            for i in range(len(indices)):
                combined = torch.cat((queries, self._get_src_single_permutation_idx(
                    indices[i], i)[-1]))  # need to fix the indexing
                ...
                for j, _ in enumerate(means_bb):
                    if j in unmatched_indices:
                        ...
                        d_feat = torch.matmul(res_feat[i],torch.t(decoder_hs[i][j]))
                        img_feat = upsaple(
                            d_feat.unsqueeze(0).unsqueeze(0))                        
                ...
                unk_label = torch.as_tensor(
                    [self.num_classes-1], device=owod_device)
        ...
        for loss in self.losses:
            ...
            losses.update(self.get_loss(loss, outputs, targets, indices,
                          num_boxes, epoch, owod_targets, owod_indices, **kwargs))

        # In case of auxiliary losses, we repeat this process with the output of each intermediate layer.
        if 'aux_outputs' in outputs:
            for a, aux_outputs in enumerate(outputs['aux_outputs']):

                if enableSSL == 3 :
                    for logits in aux_outputs :
                        if logits in ['pred_logits','pred_nc_logits','pred_boxes']:
                            aux_outputs[logits] = aux_outputs[logits][1:]
                ...
                if self.unmatched_boxes and epoch >= loss_epoch:
                    ...
                    queries = torch.arange(
                        aux_owod_outputs['pred_logits'].shape[1])
                    for i in range(len(indices)):
                        combined = torch.cat((queries, self._get_src_single_permutation_idx(
                            indices[i], i)[-1]))  # need to fix the indexing
                        uniques, counts = combined.unique(return_counts=True)
                ...
                for loss in self.losses:
                    ...
                    l_dict = self.get_loss(
                        loss, aux_outputs, targets, indices, num_boxes, epoch, owod_targets, owod_indices, **kwargs)
                    l_dict = {k + f'_{a}': v for k, v in l_dict.items()}
...
def build(args):
    ...
    # unknown class indx will not be included in the invalid class range
    invalid_cls_logits = list(range(seen_classes, num_classes-1))
    ...
    if args.NC_branch:
        weight_dict = {'loss_ce': args.cls_loss_coef,
                       'loss_NC': args.nc_loss_coef, 'loss_bbox': args.bbox_loss_coef}

    weight_dict['loss_giou'] = args.giou_loss_coef

    if enableSSL:
        weight_dict['loss_ssl'] = 0.01
    ...
    criterion = SetCriterion(args, num_classes, matcher, weight_dict,
                             losses, invalid_cls_logits, focal_alpha=args.focal_alpha)
    criterion.to(device)
